...
function generateStars(rating) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    else if (rating >= i - 0.5) stars += '<span class="star filled">★</span>';
...
function checkAuthState() {
  const user = JSON.parse(localStorage.getItem('shopease_current_user') || 'null');
  const btn = document.getElementById('authNavBtn');
  const label = document.getElementById('authNavLabel');
  if (user && btn && label) {
    label.textContent = user.firstName || 'Account';
    btn.href = '#';
    ...
    dropdown.innerHTML = `
      <div style="padding:12px 16px;border-bottom:1px solid #2a2a2a">
        <div style="color:#fff;font-weight:600">${user.firstName||''} ${user.lastName||''}</div>
        <div style="color:#888;font-size:0.75rem">${user.email||''}</div>
      </div>
      <a href="account.html#profile" class="dropdown-item">👤 My Profile</a>
      <a href="account.html#orders" class="dropdown-item">📦 My Orders</a>
      <a href="account.html#wishlist" class="dropdown-item">❤️ Wishlist</a>
      <a href="account.html#support" class="dropdown-item">🎧 Support</a>
      <div style="border-top:1px solid #2a2a2a;padding:4px">
        <a href="#" class="dropdown-item" id="logoutLink" style="color:#ef4444">🚪 Logout</a>
      </div>`;
    btn.style.position = 'relative';
    btn.appendChild(dropdown);
    dropdown.style.cssText = 'display:none;position:absolute;top:100%;right:0;background:#1a1a1a;border:1px solid #2a2a2a;border-radius:10px;min-width:200px;z-index:999;box-shadow:0 10px 30px rgba(0,0,0,0.5);margin-top:8px;overflow:hidden';
    const items = dropdown.querySelectorAll('.dropdown-item');
    items.forEach(item => {
      item.style.cssText = 'display:block;padding:10px 16px;color:#ccc;font-size:0.85rem;text-decoration:none;transition:background 0.2s';
...
function setupEventListeners() {
  ...
  checkoutBtn.addEventListener('click', () => {
    ...
    localStorage.setItem('shopease_checkout_cart', JSON.stringify(cart));
...
function openProductModal(productId) {
  ...
  const modalStars = generateStars(product.rating);
  const modalReviews = product.reviews || 100;
  modalBody.innerHTML = `
    <div class="modal-product-details">
      ...
      <div class="modal-product-info">
        ...
        <div class="product-rating">
          <div class="star-rating">${modalStars}</div>
          <span class="rating-score">${product.rating}</span>
          <span class="review-count">(${modalReviews.toLocaleString()} reviews)</span>
        </div>
        <div class="product-pricing">
          <span class="current-price">₹${product.price.toLocaleString()}</span>
          <span class="original-price">₹${product.originalPrice.toLocaleString()}</span>
